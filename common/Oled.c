...
D0   PORTC_PC12 单片机跟模块之间串接1k-2k电阻 PD1
D1   PORTC_PC13 单片机跟模块之间串接1k-2k电阻 PD0  
RST  PORTC_PC14 单片机跟模块之间串接1k-2k电阻 PD3
DC   PORTC_PC15 单片机跟模块之间串接1k-2k电阻 PD2
CS   已接地，不用接     
============================================     
...
/*****************************************************************************
 ...
 输入参数  : unsigned char *pInputImageData  
             unsigned char *pOutputImageBuf  
 输出参数  : NONE
 返 回 值  : NONE
*****************************************************************************/
...
uint16 j,usRowIdx ;  
unsigned char huancun[128];

	//清空pOutputImageBuf指向的缓存， memset库函数在string.h中
 memset(pOutputImageBuf, 0x00, 128*64);

    for (j = 0; j <=128; j++)
    {
         huancun[j] = (unsigned char)(pInputImageData[j]*64/255);
         
     ucTmpData = huancun[j] < (64 - 1) ? huancun[j] : (64 - 1);
        //当前值
        
        usRowIdx = (64 - 1) - ucTmpData;    //最大值减去当前值__列数

        (pOutputImageBuf + usRowIdx * 128)[j] = 1;
...
void tiaoyan(byte Ledge,byte Redge,byte L_edgeFlag,byte R_edgeFlag,unsigned char *pOutputImageBuf)
{
 ...
 //memset(pOutputImageBuf, 0x00, 128*64);
        ...
        if(R_edgeFlag==1)
          {r=1;}
    (pOutputImageBuf + 1*128)[Ledge] = l;
    (pOutputImageBuf + 2*128)[Ledge] = l;
    (pOutputImageBuf + 3*128)[Ledge] = l;
    (pOutputImageBuf + 4*128)[Ledge] = l; 
    (pOutputImageBuf + 1*128)[Redge] = r;
    (pOutputImageBuf + 2*128)[Redge] = r;
    (pOutputImageBuf + 3*128)[Redge] = r;
    (pOutputImageBuf + 4*128)[Redge] = r;
    
    (pOutputImageBuf + 1*128)[Ledge+1] = l;
    (pOutputImageBuf + 2*128)[Ledge+1] = l;
    (pOutputImageBuf + 3*128)[Ledge+1] = l;
    (pOutputImageBuf + 4*128)[Ledge+1] = l;
    (pOutputImageBuf + 1*128)[Redge-1] = r;
    (pOutputImageBuf + 2*128)[Redge-1] = r;
    (pOutputImageBuf + 3*128)[Redge-1] = r;
    (pOutputImageBuf + 4*128)[Redge-1] = r;
...
void middleline(byte middle,unsigned char *pOutputImageBuf)
{
  ...
  {  (pOutputImageBuf + i*128)[middle] = 1;} 
 for(i=1;i<3;i++)
    
  {
     (pOutputImageBuf + i*128)[64] = 1;
  } 
...
void thresholdline(byte threshold,unsigned char *pOutputImageBuf)
{
  int i,j; 
    ...
    Threshold=64-threshold*64/255;
    for(i=97;i<127;i++)   
    {  (pOutputImageBuf + Threshold*128)[i] = 1;}  
    for(j=0;j<30;j++)    
    {  (pOutputImageBuf + Threshold*128)[j] = 1;} 
    
     for(i=59;i<69;i++)   
    {(pOutputImageBuf + (64-175*64/255)*128)[i] = 1;}  
  
    
    for(i=59;i<69;i++)   
    {(pOutputImageBuf + (64-100*64/255)*128)[i] = 1;} 
...
void LED_PrintChar(unsigned char ucIdxX, unsigned char ucIdxY, char cData)
{
    ...
    LED_P6x8Char(ucIdxX+6, ucIdxY, i+0x30);
    LED_P6x8Char(ucIdxX+12, ucIdxY, j+0x30);
    LED_P6x8Char(ucIdxX+18, ucIdxY, k+0x30);   
...
MAIN_ConvertData(ADVH, (byte*)g_aucImageTable); //将数据转换为可以在OLED屏上显示的格式
tiaoyan(LedgeH[0],RedgeH[0],L_edgeFlagH[0],R_edgeFlagH[0],(byte*)g_aucImageTable); //寻找跳沿
middleline(midlineH,(byte*)g_aucImageTable); //寻中线
thresholdline(thresholdH,(byte*)g_aucImageTable);
LED_PrintImage((byte*)g_aucImageTable,64, 128); //在OLED上显示图像波形图 	
parameter_showH();
}